...
void* mm_malloc(size_t size) {
  ...
  // Whether there are enough free blocks in the memory size
  ptr_free_block = search_free_list(req_size);
  // Use request_more_space to enlarge the heap by the required space
  if ( ptr_free_block == NULL ){
  	request_more_space(req_size);
	ptr_free_block = search_free_list(req_size);
  }
  // Check if the last block is in use 
  preceding_block_use_tag = ptr_free_block -> size_and_tags &TAG_PRECEDING_USED;
  // Get size of the free block
  block_size = SIZE(ptr_free_block -> size_and_tags);
  
  // Subtracts the requested size from block_size, checks whether the remaining size is greater than 
  // or equal to MIN_BLOCK_SIZE, and allocates a new block with the requested size if the remaining 
  // size is greater than MIN_BLOCK_SIZE.
  if ( block_size - req_size >= MIN_BLOCK_SIZE) {
	// Set pointer to allocated memory block
	block_info*  ptr_allocate_block = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block, req_size);
	// Based on the specified number of requests (req_size), updates the header information of 
	// the free block and marks the block as being in use
	ptr_free_block-> size_and_tags = req_size | preceding_block_use_tag | TAG_USED;
	// Sets the size of the allocated memory block and a tag indicating that the previous block 
	// is in use
	ptr_allocate_block->size_and_tags = (block_size-req_size) | TAG_PRECEDING_USED;
        // Tag in footer indicating that the previous block is in use
	*((size_t*) UNSCALED_POINTER_ADD(ptr_free_block, (block_size - WORD_SIZE))) = 
		(block_size - req_size) | TAG_PRECEDING_USED;
   	insert_free_block((block_info*)UNSCALED_POINTER_ADD(ptr_free_block, req_size));    
  } else {
	// Calculates the address at the end of the allocated memory block and creates a pointer 
	// pointing to that address
	block_info*  ptr_allocate_block = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block, block_size);
	// Tag indicating ptr_free_block was used
  	ptr_free_block->size_and_tags |= TAG_USED;
	// Tag indicating that ptr_allocate_block is the preceding block of the block in use
   	ptr_allocate_block->size_and_tags |= TAG_PRECEDING_USED;
  }
...
void mm_free(void* ptr) {
  ...
  // Assign the address ptr minus WORD_SIZE to block_to_free to obtain the top address of the block 
  // to be released.
  block_to_free = (block_info*)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  // Payload size of the block to free
  payload_size = SIZE(block_to_free->size_and_tags);
  // Get the first address of the next block after the block to free
  following_block = (block_info*)UNSCALED_POINTER_ADD(ptr, payload_size-WORD_SIZE);
  // Clear tags in use from the size_and_tags field of the block to free
  block_to_free -> size_and_tags &= (~TAG_USED);
  // Clear tags in use of preceding block from the size_and_tags field of the block following the 
  // block to free
  following_block -> size_and_tags &= (~TAG_PRECEDING_USED);
  // The size and tag information of the released block is written in the header at the end of 
  // the released block
