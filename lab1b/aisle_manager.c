...
unsigned short get_section(unsigned long* aisle, int index) {
  ...
  unsigned short sec = (*aisle>>(index*16)) & SECTION_MASK;
  return sec;
}
...
unsigned short get_spaces(unsigned long* aisle, int index) {
  ...
  unsigned short spaces = sec & SPACES_MASK;
  return spaces;
}
...
unsigned short get_id(unsigned long* aisle, int index) {
  // TODO: implement this method
  unsigned short sec = get_section(aisle,index);
  unsigned short id_space = sec & ID_MASK;
  unsigned short id = id_space >> 10;
  return id;
}
...
void set_section(unsigned long* aisle, int index, unsigned short new_section) {
  // TODO: implement this method
  unsigned long new_index_spa = new_section;
  unsigned long mask = 0xFFFF;
  *aisle = (*aisle & ~(mask<<index*16)) | (new_index_spa<< index*16);
}
...
void set_spaces(unsigned long* aisle, int index, unsigned short new_spaces) {
  // TODO: implement this method
  unsigned long new_index_space = new_spaces;
  unsigned long mask = 0x03FF;
  unsigned long zero_check=0xFC00;
  if (!(new_index_space & zero_check)) {
    *aisle = (*aisle & ~(mask<<index*16)) | (new_index_space<< index*16);
  }
}
...
void set_id(unsigned long* aisle, int index, unsigned short new_id) {
  // TODO: implement this method
  unsigned long mask = 0xFC00;
  unsigned long index_id = new_id;
  unsigned long new_index_id = index_id << 10;
  if( !(index_id&~(0x3F))&&!(new_index_id&0x03FF)){
      *aisle = (*aisle & ~(mask<<index*16)) | (new_index_id<<index*16);
    }
}
...
void toggle_space(unsigned long* aisle, int index, int space_index) {
  // TODO: implement this method
  unsigned short curr_section= get_section(aisle,index);
  unsigned short next_section= (1<<space_index) ^ curr_section;
  set_section(aisle,index,next_section);
}
...
void add_items(unsigned long* aisle, int index, int n) {
  // TODO: implement this method
  int available_space = 10 - num_items(aisle,index);
  unsigned short space = get_spaces(aisle,index);
...
void remove_items(unsigned long* aisle, int index, int n) {
  // TODO: implement this method
  unsigned short item_num = num_items(aisle,index);
  unsigned short section, new_section;
  unsigned short space = get_spaces(aisle,index);
  if ( n > item_num) {
    n = item_num;
  }
...
void rotate_items_left(unsigned long* aisle, int index, int n) {
  ...
  unsigned short space = get_spaces(aisle, index);
  unsigned short insert_space = space >> (10-n);
  unsigned short exist_space = space << n;
  unsigned short new_space = ( insert_space| exist_space)&SPACES_MASK;
  set_spaces(aisle,index,new_space);
}
